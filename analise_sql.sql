...
select count(id_chamado) as quantidade_chamados,
...
group by id_tipo,
  id_subtipo,
  tipo,
  subtipo -- na query acima, conseguimos ver uma lista de chamados relacionados ao tipo Onibus que não possuem nenhum bairro atrelado.
  ---
...
select chamados.subtipo,
  count(chamados.id_chamado) as quantidade_chamados
from datario.administracao_servicos_publicos.chamado_1746 as chamados
where chamados.subtipo = "Perturbação do sossego"
  and chamados.data_inicio between '2022-01-01' and '2023-12-31'
...
from datario.administracao_servicos_publicos.chamado_1746 as chamados
  join datario.turismo_fluxo_visitantes.rede_hoteleira_ocupacao_eventos as eventos on chamados.data_inicio between eventos.data_inicial and eventos.data_final
...
select eventos.evento,
  ...
  count(chamados.id_chamado) / (
    extract(
      day
      from (eventos.data_final - eventos.data_inicial)
    ) + 1
  ) as media_diaria_chamados,
  count(chamados.id_chamado) as quantidade_chamados,
  extract(
    day
    from eventos.data_final - eventos.data_inicial
...
(
  select eventos.evento as periodo,
    extract(
      year
      from eventos.data_inicial
    ) as ano,
    ...
    ) / total.media_diaria_chamados as proporcao,
    count(chamados.id_chamado) as quantidade_chamados,
    extract(
      day
      from eventos.data_final - eventos.data_inicial
  ...
  from datario.administracao_servicos_publicos.chamado_1746 as chamados,
    (
      select 'Média Total 2022-2023' as periodo,
        count(id_chamado) / (
          extract(
            day
            from '2023-12-31' - '2022-01-01'
          ) + 1
        ) as media_diaria_chamados,
        count(id_chamado) as quantidade_chamados,
        extract(
